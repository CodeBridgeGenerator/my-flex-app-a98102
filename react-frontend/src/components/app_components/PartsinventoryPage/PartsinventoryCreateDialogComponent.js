...
const PartsinventoryCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            partName: _entity?.partName,description: _entity?.description,quantityInStock: _entity?.quantityInStock,price: _entity?.price,supplierId: _entity?.supplierId?._id,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Partsinventory updated successfully" });
