...
const LoyaltyprogramsFakerDialogComponent = (props) => {
    ...
    const [supplierIdIds, setsupplierIdIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("suppliers")
                        .find({ query: { $select: ["_id","supplierName"] } })
                        .then((data) => {
                        setsupplierIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = loyaltyprogramsFakerFactory(props.user,fakerCount,supplierIdIds);
