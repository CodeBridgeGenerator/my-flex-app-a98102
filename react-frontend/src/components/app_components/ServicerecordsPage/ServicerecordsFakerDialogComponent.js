...
const [serviceIdIds, setserviceIdIds] = useState();
...
const [technicianIdIds, settechnicianIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("invoices")
                        .find({ query: { $select: ["_id","serviceDate"] } })
                        .then((data) => {
...
client.service("technicians")
    ...
    const onRun = async () => {
        let fakeData = servicerecordsFakerFactory(props.user,fakerCount,invoiceIdIds,serviceIdIds,vehicleIdIds,technicianIdIds);
